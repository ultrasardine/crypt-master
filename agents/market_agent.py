...
class MarketAnalysisAgent:
    ...
    async def _get_sentiment_score(self) -> IndicatorScore | None:
        ...
        try:
            ...
            if result.data_unavailable:
                logger.warning(
                    f"Sentiment data unavailable: {result.error_message}"
                )
                return None
    ...
    async def _get_news_score(self) -> IndicatorScore | None:
        ...
        try:
            ...
            if result.ollama_unavailable:
                logger.warning(
                    f"News analysis unavailable (Ollama down): {result.error_message}"
                )
                return None
            
            if result.data_unavailable:
                logger.warning(
                    f"News data unavailable: {result.error_message}"
                )
                return None
    ...
    async def analyze_symbol(
        ...
        if news_score is not None:
            conflict = self._detect_news_technical_conflict(
                technical_scores, news_score
            )
            if conflict:
                logger.warning(
                    f"News sentiment conflicts with technical signals for {symbol}"
                )
    ...
    def _calculate_technical_scores(
        ...
        # Stochastic
        stoch_result = self._technical_analyzer.calculate_stochastic(
            highs, lows, closes
        )
        scores.append(IndicatorScore(
            name="STOCH",
    ...
    async def store_signal(self, signal: Signal) -> None:
        ...
        try:
            ...
            trading_pair, _ = TradingPair.objects.get_or_create(
                symbol=signal.symbol,
                defaults={
                    "base_currency": signal.symbol.split("_")[0] if "_" in signal.symbol else signal.symbol[:3],
                    "quote_currency": signal.symbol.split("_")[1] if "_" in signal.symbol else "USDT",
                    "is_active": True,
                },
            )
