...
class PionexConfig:
    ...
    def validate_for_live_trading(self) -> None:
        ...
        """
        if not self.api_key:
            raise ConfigurationError("API key is required for live trading", "pionex.api_key")
        if not self.api_secret:
            raise ConfigurationError("API secret is required for live trading", "pionex.api_secret")
...
class AnalysisConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rsi_period < 1:
            raise ConfigurationError(
                f"rsi_period must be at least 1, got {self.rsi_period}", "analysis.rsi_period"
            )
        if self.macd_fast < 1:
            raise ConfigurationError(
                f"macd_fast must be at least 1, got {self.macd_fast}", "analysis.macd_fast"
            )
        if self.macd_slow < 1:
            raise ConfigurationError(
                f"macd_slow must be at least 1, got {self.macd_slow}", "analysis.macd_slow"
            )
        ...
        if self.macd_signal < 1:
            raise ConfigurationError(
                f"macd_signal must be at least 1, got {self.macd_signal}", "analysis.macd_signal"
            )
...
class LLMConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and not self.ollama_url:
            raise ConfigurationError("ollama_url is required when LLM is enabled", "llm.ollama_url")
        if self.enabled and not self.model:
            raise ConfigurationError("model is required when LLM is enabled", "llm.model")
...
class LoggingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_bytes < 1024:
            raise ConfigurationError(
                f"max_bytes must be at least 1024, got {self.max_bytes}", "logging.max_bytes"
            )
    ...
    for env_var, (config_path, converter) in env_mappings.items():
        ...
        if env_value is not None:
            try:
                ...
                raise ConfigurationError(
                    f"Invalid value for environment variable {env_var}: {e}", config_path
                )
