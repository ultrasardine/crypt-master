...
class PositionSizer:
    ...
    def calculate_position_size(
        ...
        was_capped = (
            kelly_fraction > 0
            and kelly_fraction * self.config.kelly_fraction_multiplier
...
class DrawdownTracker:
    ...
    def calculate_drawdown(self) -> float:
        ...
        return float((self._high_water_mark - self._current_value) / self._high_water_mark)
...
class RiskManager:
    ...
    def update_portfolio(
        ...
        if bot_allocation is not None:
            if bot_allocation < 0:
                raise ValueError(f"bot_allocation must be non-negative, got {bot_allocation}")
    ...
    def validate_trade(
        ...
        if not self.is_trading_allowed():
            rejection_reasons.append("Trading is halted due to drawdown limit breach")
        ...
        max_position_value = self._position_sizer.get_max_position_value(self._portfolio_value)
    ...
    def validate_bot_creation(
        ...
        if allocation_amount <= 0:
            rejection_reasons.append(f"Allocation amount must be positive, got {allocation_amount}")
    ...
    def get_available_bot_allocation(self) -> Decimal:
        ...
        Requirements:
            - 10.7: Validate that allocated funds for bot creation do not exceed portfolio risk limits
        """
        max_allocation = self._portfolio_value * Decimal(str(self.config.max_bot_allocation_pct))
