...
class Backtester:
    ...
    def _close_position(
        ...
        entry_slippage = (
            abs(self._position_entry_price - exit_price)
            * Decimal(str(self.config.slippage_pct))
            * quantity
        )
    ...
    def _calculate_metrics(self, candles: list[Candle]) -> BacktestMetrics:
        ...
        # Total return
        total_return = (
            float((self._cash - self.config.initial_capital) / self.config.initial_capital) * 100
        )
