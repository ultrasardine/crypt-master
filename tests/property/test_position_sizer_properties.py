    ...
    def test_kelly_negative_when_no_edge(
        ...
        **Validates: Requirements 6.1**
        ...
        if has_positive_edge:
            assert kelly > 0, (
                f"Kelly should be positive when edge exists: "
                f"kelly={kelly}, p*b={p*b}, q={q}"
            )
        elif (p * b) < q:
            assert kelly < 0, (
                f"Kelly should be negative when no edge: "
                f"kelly={kelly}, p*b={p*b}, q={q}"
            )
        else:
    ...
    **Validates: Requirements 6.2**
    ...
    def test_position_is_non_negative(
        ...
        **Validates: Requirements 6.2**
        ...
        assert result.position_size >= Decimal("0"), (
            f"Position size {result.position_size} is negative "
    ...
    def test_was_capped_flag_accuracy(
        ...
        **Validates: Requirements 6.2**
        ...
        kelly_after_multiplier = (
            result.kelly_fraction * sizer.config.kelly_fraction_multiplier
        )
        should_be_capped = (
            result.kelly_fraction > 0 and
            kelly_after_multiplier > sizer.max_position_pct
        )
    ...
    def test_calculation_is_deterministic(
        ...
        **Validates: Requirements 6.1, 6.2**
        ...
        assert result1.position_size == result2.position_size, (
            f"Position size not deterministic: "
