    ...
    def test_allocation_percent_calculation(
        ...
        **Validates: Requirements 10.7**
        ...
        assert result.allocation_percent == pytest.approx(expected_percent, rel=1e-9), (
            f"allocation_percent {result.allocation_percent} != "
    ...
    def test_cumulative_allocations_enforced(
        ...
        **Validates: Requirements 10.7**
        ...
        assert result3.is_valid is False, (
            f"Third allocation should be rejected (would exceed limit)"
        )
    ...
    **Validates: Requirements 10.7**
    ...
    def test_just_over_limit_is_rejected(
        ...
        **Validates: Requirements 10.7**
        ...
        assert result.is_valid is False, (
            f"Allocation just over the limit should be rejected"
        )
    ...
    def test_result_contains_all_required_fields(
        ...
        **Validates: Requirements 10.7**
        ...
        assert isinstance(result.current_bot_allocation, Decimal), "current_bot_allocation should be Decimal"
        assert isinstance(result.new_total_allocation, Decimal), "new_total_allocation should be Decimal"
        ...
        assert isinstance(result.max_allocation_percent, float), "max_allocation_percent should be float"
    ...
    **Validates: Requirements 10.7**
    ...
    def test_bot_rejected_when_trading_halted(
        ...
        **Validates: Requirements 10.7**
        ...
        assert result.is_valid is False, (
            "Bot creation should be rejected when trading is halted"
        )
    ...
    def test_bot_allowed_after_drawdown_reset(
        ...
        **Validates: Requirements 10.7**
        ...
        if float(new_allocation_amount / breached_value) <= 0.50:
            assert result.is_valid is True, (
                f"Bot creation should be allowed after reset. "
                f"Rejection: {result.rejection_reasons}"
            )
