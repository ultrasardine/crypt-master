...
symbol_strategy = st.sampled_from(["BTC_USDT", "ETH_USDT", "SOL_USDT", "XRP_USDT", "ADA_USDT"])
    ...
    def test_buy_order_balance_changes(
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        assert order.status == SimulatedOrderStatus.FILLED, (
            "Order should be filled with sufficient balance"
        )
    ...
    def test_insufficient_balance_rejects_order(
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        assert order.status == SimulatedOrderStatus.REJECTED, (
            "Order should be rejected with insufficient balance"
        )
    ...
    def test_ledger_entries_match_balance_changes(
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        if order.status == SimulatedOrderStatus.FILLED:
            ...
            assert len(new_entries) == 2, f"Expected 2 new ledger entries, got {len(new_entries)}"

            # Verify entries have correct reference ID
            for entry in new_entries:
                assert entry.reference_id == order.order_id, "Ledger entry reference_id mismatch"
    ...
    **Validates: Requirements 2.1, 10.12**
    ...
    def test_simulator_instances_are_isolated(
        ...
        **Validates: Requirements 10.12**
        ...
        # Verify isolation
        assert sim1.get_balance("USDT").free == balance1, "Simulator 1 balance incorrect"
        assert sim2.get_balance("USDT").free == balance2, "Simulator 2 balance incorrect"
    ...
    def test_simulated_orders_do_not_persist_after_reset(
        ...
        **Validates: Requirements 10.12**
        ...
        assert len(simulator.get_all_balances()) == 0, "Balances should be cleared after reset"
        assert len(simulator.get_orders()) == 0, "Orders should be cleared after reset"
        assert len(simulator.get_trades()) == 0, "Trades should be cleared after reset"
    ...
    def test_bot_operations_are_simulated(
        ...
        **Validates: Requirements 10.12**
        ...
        assert balance_after_create.locked == investment, "Locked balance should equal investment"
    ...
    def test_ledger_changes_sum_to_final_balance(
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        assert final_balance == usdt_changes, (
            f"Sum of ledger changes ({usdt_changes}) doesn't equal final balance ({final_balance})"
        )
